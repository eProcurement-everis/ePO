...
Codes have been implemented as link:https://www.w3.org/2006/07/SWD/wiki/SkosDesign/SKOS-XL.html[SKOS-XL] concepts, which allows to specify
metadata about each concept thus making possible to set values for the attributes defined in the Core Component Technical Specification (CCTS),
e.g. listAgencyID, listVersionID, etc. (see next section below, on codes and code lists).
